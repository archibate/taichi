...
res = 128  # 600 for a larger resoultion
dt = 0.03
max_mgpcg_iters = 8  # 4 for quicker but not-so-accurate result
f_strength = 10000.0
...
@ti.kernel
def init_solver_bc():
    for i, j in velocity_divs:
        solver.init_bc(ti.Vector([i, j]), velocity_divs[i, j] * 0.25)
...
def step(mouse_data):
    ...
    init_solver_bc()
    solver.solve(max_mgpcg_iters)

    subtract_gradient(velocities_pair.cur, solver.x)
